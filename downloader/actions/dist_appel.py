...
from downloader.pages.reporting_dist_appel import ReportingDistributionAppelPage
...
def run(driver):
    """Scénario : Télécharger distribution d'appels"""
    ...
    dist_appel = ReportingDistributionAppelPage(driver)
    reporting_page = ReportingPage(driver)
    reporting_page.open_saved_reports()
    time.sleep(2)
    dist_appel.click_dist_appel()
    time.sleep(2)
    reporting_page.click_ouvrir()
    time.sleep(2)
    dist_appel.generate_button()
    ...
    time.sleep(15)
    task_list_page.click_delete_history_button()
    time.sleep(5)
    task_list_page.click_delete_all()
    time.sleep(5)